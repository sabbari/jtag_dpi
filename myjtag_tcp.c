...
extern int jtag_server(int *tck, int *tms, int *tdi,
  ...
  case (RECEIVE):
    ...
    state = size > 0 ? SEND_JTAG : RECEIVE;
    return 1;
  ...
  case (SEND_JTAG):
        ...
        *tdi = (command & 2) != 0;
        *tck = (command & 8) != 0;
        if(command&4)state=SEND_TOCLIENT;
        return 1;

  case (SEND_TOCLIENT):
    miso_buffer[0]=tdo&1;
    state = RECEIVE;
    int sent = send_to_client(clientFd, miso_buffer, 1);
    if (sent < 1) {
      perror("Couldn't send all tdo buffer to client ");
      return -1;
    }
    return 1;
  }
